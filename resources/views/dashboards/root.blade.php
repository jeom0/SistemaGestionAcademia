...
<div class="flex flex-col gap-10 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-outline shadow-sm relative overflow-hidden">
        <div class="relative z-10 flex flex-col gap-2">
            <h1 class="text-2xl md:text-4xl font-black text-secondary tracking-tight">Bienvenido, {{ auth()->user()->name }}</h1>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2.5rem] shadow-premium hover-lift group">
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-4xl font-black text-secondary mt-2 tracking-tighter">{{ \App\Models\User::count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2.5rem] shadow-premium hover-lift group">
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-4xl font-black text-secondary mt-2 tracking-tighter">{{ \App\Models\User::where('status', 'activo')->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2.5rem] shadow-premium hover-lift group">
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-4xl font-black text-secondary mt-2 tracking-tighter">{{ \App\Models\Movement::where('status', 'pendiente')->count() }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div class="bg-white border border-outline rounded-[2.5rem] shadow-premium overflow-hidden hover-lift group">
            <div class="p-8 border-b border-outline flex justify-between items-center bg-gray-50/30">
                ...
                <a href="{{ route('root.users.index') }}" class="text-sm font-bold text-primary hover:underline">Ver todos</a>
            </div>
            <div class="p-8">
                <div class="flex flex-col gap-6">
                        ...
                        <div class="flex items-center justify-between group/item p-2 rounded-2xl hover:bg-surface-variant transition-all">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[10px] text-on-surface-variant uppercase font-black tracking-widest">{{ $user->role }}</span>
                                </div>
                            </div>
                            <span class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest {{ $user->status === 'activo' ? 'bg-emerald-100 text-primary' : 'bg-red-50 text-red-700' }}">
                                {{ $user->status }}
                            </span>
                        </div>
                    @endforeach
        ...
        <div class="bg-white border border-outline rounded-[2.5rem] shadow-premium overflow-hidden flex flex-col hover-lift group">
            <div class="p-8 border-b border-outline flex justify-between items-center bg-gray-50/30">
                ...
                <a href="{{ route('movements.index') }}" class="text-sm font-bold text-primary hover:underline">Ver Historial</a>
            </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    @foreach($recentMovements as $m)
                        <div class="flex items-center justify-between group">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl {{ $m->type === 'ingreso' ? 'bg-emerald-50 text-primary' : 'bg-red-50 text-red-600' }} flex items-center justify-center shrink-0">
                            ...
                            <div class="flex flex-col items-end">
                                ...
                                <span class="text-[9px] font-bold uppercase {{ $m->status === 'completado' ? 'text-emerald-500' : 'text-amber-500' }}">
                                    {{ $m->status }}
                                </span>
                            </div>
                        </div>
