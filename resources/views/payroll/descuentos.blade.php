...
@section('title', 'Descuentos de Empleados')
...
<div class="flex flex-col gap-10 max-w-[1400px] mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-6 md:p-10 rounded-[2.5rem] border border-outline shadow-sm relative overflow-hidden">
        <div class="relative z-10 flex flex-col gap-2">
            <h1 class="text-2xl md:text-4xl font-black text-secondary tracking-tight">Descuentos</h1>
            <p class="text-xs md:text-sm text-on-surface-variant font-medium">Gestión y visualización de descuentos y deducciones de empleados.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="p-8 bg-gradient-to-br from-rose-50 to-rose-100/50 border border-rose-100 rounded-[2.5rem] shadow-sm hover-lift group">
            <div class="flex flex-col gap-6">
                <div class="w-14 h-14 rounded-2xl bg-rose-600 text-white flex items-center justify-center group-hover:scale-110 transition-all shadow-sm">
                    <span class="material-symbols-outlined text-[28px]">money_off</span>
                </div>
                <div class="flex flex-col">
                    <span class="text-[11px] font-black text-rose-800 uppercase tracking-[0.2em] opacity-80">Total Descuentos (Mes)</span>
                    <span class="text-4xl font-black text-rose-900 mt-2 tracking-tighter">${{ number_format($descuentos->sum('amount'), 0) }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-outline rounded-[2.5rem] shadow-premium overflow-hidden">
        <div class="p-8 border-b border-outline flex justify-between items-center bg-gray-50/50">
            <h3 class="text-xl font-bold text-secondary">Historial de Descuentos</h3>
        </div>
                ...
                <tbody class="divide-y divide-outline">
                    ...
                    <tr class="hover:bg-surface transition-all">
                        <td class="px-8 py-5 text-xs font-bold text-on-surface-variant">
                            {{ $descuento->date->format('d M, Y') }}
                        </td>
                        <td class="px-8 py-5">
                            <span class="text-sm font-black text-secondary">{{ $descuento->description }}</span>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs">
                                    {{ substr($descuento->user->name, 0, 1) }}
                                </div>
                                <span class="text-xs font-bold text-secondary">{{ $descuento->user->name }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-5 text-right font-black text-rose-600">
                            -${{ number_format($descuento->amount, 0) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-16 text-center">
                            <div class="flex flex-col items-center justify-center gap-4 text-on-surface-variant opacity-60">
                                <span class="material-symbols-outlined text-6xl">money_off</span>
                                <p class="text-sm font-bold">No hay registros de descuentos aún.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
