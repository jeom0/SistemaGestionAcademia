...
<div class="flex flex-col gap-10 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('root.users.create') }}" class="h-12 px-6 bg-primary text-white rounded-2xl font-bold flex items-center gap-2 hover:shadow-xl transition-all active:scale-[0.98]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div class="p-8 bg-white border border-outline rounded-[2rem] shadow-sm">
            <div class="flex items-center gap-6">
                ...
                <div class="flex flex-col">
                    <span class="text-3xl font-black text-secondary">{{ $users->count() }}</span>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2rem] shadow-sm">
            <div class="flex items-center gap-6">
                ...
                <div class="flex flex-col">
                    <span class="text-3xl font-black text-secondary">{{ $users->where('role', 'administrador')->count() }}</span>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2rem] shadow-sm">
            <div class="flex items-center gap-6">
                ...
                <div class="flex flex-col">
                    <span class="text-3xl font-black text-secondary">{{ $users->where('role', 'colaborador')->count() }}</span>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2rem] shadow-sm">
            <div class="flex items-center gap-6">
                ...
                <div class="flex flex-col">
                    <span class="text-3xl font-black text-secondary">{{ $users->where('status', 'activo')->count() }}</span>
    ...
    <div class="bg-white border border-outline rounded-[2.5rem] shadow-sm overflow-hidden">
                        ...
                        <tr class="hover:bg-surface transition-all group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold text-secondary">{{ $user->name }}</span>
                                        <span class="text-xs text-on-surface-variant font-medium">{{ $user->email }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest {{ $user->role === 'administrador' ? 'bg-blue-50 text-blue-700' : ($user->role === 'root' ? 'bg-purple-50 text-purple-700' : 'bg-amber-50 text-amber-700') }}">
                                    {{ $user->role }}
                                </span>
                            </td>
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-2.5 h-2.5 rounded-full {{ $user->status === 'activo' ? 'bg-emerald-500 shadow-lg shadow-emerald-500/50' : 'bg-red-500 shadow-lg shadow-red-500/50' }}"></div>
                                    <span class="text-xs font-bold text-secondary uppercase tracking-widest">{{ $user->status }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                    <a href="{{ route('root.users.edit', $user) }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-outline text-on-surface-variant hover:text-primary hover:border-primary transition-all shadow-sm">
                                        <span class="material-symbols-outlined text-[20px]">edit</span>
                                    </a>
                                    
                                    <form action="{{ route('root.users.toggle-status', $user) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-outline {{ $user->status === 'activo' ? 'text-red-500 hover:border-red-500' : 'text-emerald-500 hover:border-emerald-500' }} transition-all shadow-sm">
                                            <span class="material-symbols-outlined text-[20px]">{{ $user->status === 'activo' ? 'block' : 'check_circle' }}</span>
                                        </button>
                                    </form>

                                    <form action="{{ route('root.users.destroy', $user) }}" method="POST" onsubmit="return confirm('¿Eliminar usuario?')">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="p-8 bg-gray-50/30 border-t border-outline flex justify-between items-center">
            <span class="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Mostrando {{ $users->count() }} usuarios</span>
