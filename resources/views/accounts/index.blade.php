...
<div class="flex flex-col gap-10 max-w-[1400px] mx-auto" x-data="{ modalOpen: false, activeAccountName: '', activeAccountBalance: 0, activeAccountMovements: [] }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="bg-white border border-outline rounded-[2.5rem] p-10 shadow-premium hover-lift group cursor-pointer transition-all hover:border-primary/30"
             @click="activeAccountName = '{{ addslashes($account->associated_to) }}'; 
                     activeAccountBalance = {{ $account->balance }}; 
                     activeAccountMovements = {{ json_encode($movementsGrouped[$account->associated_to] ?? []) }};
                     modalOpen = true; 
            ...
            <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col">
                    <h3 class="text-xl font-black text-secondary uppercase tracking-tight truncate w-full">{{ $account->associated_to }}</h3>
                ...
                <div class="pt-6 border-t border-outline flex items-end justify-between">
                    <span class="text-3xl font-black {{ $account->balance >= 0 ? 'text-primary' : 'text-red-600' }}">
