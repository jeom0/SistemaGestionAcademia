...
<div class="flex flex-col gap-10 max-w-[1400px] mx-auto no-print" x-data="{ activeTab: 'movements' }">
    ...
    <div class="bg-white border border-outline rounded-[2.5rem] p-8 shadow-premium">
        <form action="{{ route('reports.index') }}" method="GET" class="flex flex-col gap-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <!-- Date range from -->
                <div class="flex flex-col gap-2">
                    <label class="text-[11px] font-bold text-on-surface-variant uppercase tracking-widest ml-1">Fecha Desde</label>
                    <input type="date" name="from_date" value="{{ request('from_date') }}" class="w-full h-12 px-4 rounded-xl bg-surface-variant/30 border border-outline focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all text-secondary font-medium" />
                </div>
                <!-- Date range to -->
                <div class="flex flex-col gap-2">
                    <label class="text-[11px] font-bold text-on-surface-variant uppercase tracking-widest ml-1">Fecha Hasta</label>
                    <input type="date" name="to_date" value="{{ request('to_date') }}" class="w-full h-12 px-4 rounded-xl bg-surface-variant/30 border border-outline focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all text-secondary font-medium" />
                </div>
            ...
            <div class="flex justify-end gap-3 border-t border-outline pt-6">
                <a href="{{ route('reports.index') }}" class="h-12 px-6 rounded-xl border border-outline text-on-surface-variant font-bold hover:bg-surface-variant flex items-center justify-center transition-all text-sm">
                    Limpiar Filtros
                </a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="p-8 bg-white border border-outline rounded-[2.5rem] shadow-premium hover-lift group">
            <div class="flex flex-col gap-4">
                ...
                <h3 class="text-3xl font-black text-primary tracking-tighter">+${{ number_format($ingresos, 2) }}</h3>
            </div>
        </div>
        ...
        <div class="p-8 bg-white border border-outline rounded-[2.5rem] shadow-premium hover-lift group">
            <div class="flex flex-col gap-4">
                ...
                <h3 class="text-3xl font-black text-red-600 tracking-tighter">-${{ number_format($egresos, 2) }}</h3>
            </div>
        </div>
        ...
        <div class="p-8 bg-secondary border border-secondary/20 rounded-[2.5rem] shadow-2xl shadow-secondary/15 hover-lift text-white">
            <div class="flex flex-col gap-4">
                ...
                <h3 class="text-3xl font-black tracking-tighter">${{ number_format($balance, 2) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 bg-surface-variant/20 border border-outline rounded-[2rem] p-6 md:p-8">
        ...
        <div class="flex flex-col gap-1 border-b md:border-b-0 md:border-r border-outline pb-4 md:pb-0 md:pr-4">
            ...
            @endphp
            <span class="text-xl font-black {{ $margin >= 0 ? 'text-primary' : 'text-red-500' }}">{{ number_format($margin, 1) }}%</span>
        ...
        <div class="flex flex-col gap-1 border-b md:border-b-0 md:border-r border-outline pb-4 md:pb-0 md:px-4">
            ...
            <span class="text-xl font-black text-secondary">{{ $movements->count() }}</span>
        ...
        <div class="flex flex-col gap-1 border-b md:border-b-0 md:border-r border-outline pb-4 md:pb-0 md:px-4">
            ...
            @endphp
            <span class="text-xl font-black text-primary">${{ number_format($avgIncome, 0) }}</span>
        ...
        <div class="flex flex-col gap-1 px-4">
            ...
            @endphp
            <span class="text-xl font-black text-red-500">${{ number_format($avgExpense, 0) }}</span>
    ...
    <div class="bg-white border border-outline rounded-[2.5rem] shadow-premium overflow-hidden">
        ...
        <div class="border-b border-outline bg-gray-50/20 px-8 py-5 flex justify-between items-center flex-wrap gap-4">
            ...
            <span class="text-xs text-on-surface-variant font-medium" x-text="activeTab === 'movements' ? '{{ $movements->count() }} movimientos' : '{{ $auditLogs->count() }} bitácoras'"></span>
        </div>
                    ...
                    <tr class="hover:bg-surface transition-all">
                        ...
                        <td class="px-8 py-5">
                            <div class="flex flex-col gap-0.5 max-w-xs">
                                <span class="text-xs font-black text-secondary">{{ $m->description }}</span>
                                <span class="text-[9px] font-bold uppercase tracking-wider {{ $m->type === 'ingreso' ? 'text-primary' : 'text-red-500' }}">{{ $m->type }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-5">
                            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider {{ $m->status === 'completado' ? 'bg-emerald-50 text-primary border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100' }}">
                                {{ $m->status }}
                            </span>
                        </td>
                        <td class="px-8 py-5 text-xs font-black text-right {{ $m->type === 'ingreso' ? 'text-primary' : 'text-red-600' }}">
                        ...
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-secondary">{{ $log->user ? $log->user->name : 'Sistema' }}</span>
                                    <span class="text-[9px] text-on-surface-variant uppercase font-bold tracking-wider">{{ $log->user ? $log->user->role : 'root' }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-5">
                            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider {{ $log->action === 'created' ? 'bg-emerald-50 text-primary border border-emerald-100' : ($log->action === 'deleted' ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-blue-50 text-blue-600 border border-blue-100') }}">
                                {{ $log->action }}
                            </span>
                        </td>
                        <td class="px-8 py-5 text-xs font-bold text-on-surface-variant">
...
<div class="print-only p-10 flex flex-col gap-8 text-black" style="display: none;">
    ...
    <div class="flex justify-between items-center border-b-4 border-emerald-800 pb-6">
        <div class="flex flex-col">
            ...
            <span class="text-xs text-slate-400 mt-1">Generado automáticamente por Antigravity AI el {{ date('d/m/Y - h:i A') }}</span>
        </div>
        <div class="text-right flex flex-col justify-end">
            ...
            <span class="text-[10px] font-bold text-slate-500 mt-1 uppercase tracking-wide">
                Filtros: {{ request('account') ? 'Cuenta: ' . request('account') : 'Todas las cuentas' }} 
                {{ request('from_date') ? '• Desde: ' . request('from_date') : '' }} 
                {{ request('to_date') ? '• Hasta: ' . request('to_date') : '' }}
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-6 text-center">
        <div class="border border-slate-300 p-6 rounded-2xl flex flex-col gap-1">
            ...
            <h2 class="text-2xl font-black text-emerald-700">+${{ number_format($ingresos, 2) }}</h2>
        </div>
        <div class="border border-slate-300 p-6 rounded-2xl flex flex-col gap-1">
            <span class="text-[9px] font-bold uppercase text-slate-500 tracking-wider">Total Egresos</span>
            <h2 class="text-2xl font-black text-red-600">-${{ number_format($egresos, 2) }}</h2>
        </div>
        <div class="border border-slate-300 p-6 rounded-2xl flex flex-col gap-1" style="background-color: #f8fafc;">
            ...
            <h2 class="text-2xl font-black text-slate-800">${{ number_format($balance, 2) }}</h2>
        </div>
    </div>
    ...
    <div class="border border-slate-300 p-6 rounded-2xl flex flex-col gap-4">
        ...
        <div class="grid grid-cols-4 gap-4 text-center">
            <div class="flex flex-col">
                ...
                <span class="text-sm font-black">{{ number_format($margin, 2) }}%</span>
            </div>
            <div class="flex flex-col">
                <span class="text-[9px] font-bold text-slate-500 uppercase">Volumen</span>
                <span class="text-sm font-black">{{ $movements->count() }} transacciones</span>
            </div>
            <div class="flex flex-col">
                <span class="text-[9px] font-bold text-slate-500 uppercase">Ingreso Promedio</span>
                <span class="text-sm font-black">${{ number_format($avgIncome, 2) }}</span>
            </div>
            <div class="flex flex-col">
                <span class="text-[9px] font-bold text-slate-500 uppercase">Gasto Promedio</span>
                <span class="text-sm font-black">${{ number_format($avgExpense, 2) }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-3">
        ...
        <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr>
                    <td class="py-2 px-3 font-semibold">{{ $m->date->format('d/m/Y') }}</td>
                    <td class="py-2 px-3 font-black">{{ $m->description }} ({{ $m->type }})</td>
                    <td class="py-2 px-3">{{ $m->associated_to ?? 'General' }}</td>
                    <td class="py-2 px-3">{{ $m->user ? $m->user->name : 'Sistema' }}</td>
                    <td class="py-2 px-3 text-right font-bold {{ $m->type === 'ingreso' ? 'text-emerald-700' : 'text-red-700' }}">
                        {{ $m->type === 'ingreso' ? '+' : '-' }}${{ number_format($m->amount, 2) }}
                    </td>
                </tr>
                @endforeach
    ...
    <div class="grid grid-cols-2 gap-10 mt-16 text-center text-xs">
        <div class="flex flex-col gap-1">
            ...
            <span class="font-black text-slate-800">{{ auth()->user()->name }}</span>
            <span class="text-[9px] text-slate-500 uppercase tracking-wide">Firma Responsable ({{ auth()->user()->role }})</span>
        </div>
        <div class="flex flex-col gap-1">
            <div class="border-b border-slate-500 w-48 mx-auto h-10"></div>
...
<script>
    function exportToExcel() {
        ...
        const summaryData = [
            ...
            ["Margen de Operación", "{{ number_format($margin, 2) }}%"],
        ...
        @foreach($movements as $m)
            summaryData.push([
                ...
                "{{ $m->type }}",
                "{{ str_replace('"', '""', $m->associated_to ?? 'General') }}",
                {{ $m->amount }}
            ]);
        @endforeach
