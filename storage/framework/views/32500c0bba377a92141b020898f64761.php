...
<div class="flex flex-col gap-10 max-w-[1200px] mx-auto">
    ...
    <div class="flex justify-between items-center">
        <div class="flex flex-col gap-1">
            ...
            <p class="text-on-surface-variant font-medium">Actualice la información de acceso de <span class="font-bold text-secondary"><?php echo e($user->name); ?></span>.</p>
        </div>
        <a href="<?php echo e(route('root.users.index')); ?>" class="h-12 px-6 border border-outline text-secondary rounded-2xl font-bold flex items-center gap-2 hover:bg-surface-variant transition-all active:scale-[0.98]">
    ...
    <div class="bg-white border border-outline rounded-[2.5rem] shadow-sm overflow-hidden">
        <div class="p-12">
            ...
            <form action="<?php echo e(route('root.users.update', $user)); ?>" method="POST" class="flex flex-col gap-10">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="role" class="w-full h-14 px-5 rounded-2xl bg-surface-variant/30 border border-outline focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all text-secondary font-medium appearance-none" required>
                            ...
                            <option value="colaborador" <?php echo e(old('role', $user->role) == 'colaborador' ? 'selected' : ''); ?>>Colaborador</option>
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="status" class="w-full h-14 px-5 rounded-2xl bg-surface-variant/30 border border-outline focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all text-secondary font-medium appearance-none" required>
                            <option value="activo" <?php echo e(old('status', $user->status) == 'activo' ? 'selected' : ''); ?>>Activo</option>
                            <option value="inactivo" <?php echo e(old('status', $user->status) == 'inactivo' ? 'selected' : ''); ?>>Inactivo</option>
                        </select>
                    </div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\PC\Desktop\conduser\resources\views/users/edit.blade.php ENDPATH**/ ?>