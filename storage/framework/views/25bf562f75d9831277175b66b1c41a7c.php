    ...
    <div class="w-full max-w-[500px] bg-white rounded-[2.5rem] shadow-premium border border-outline overflow-hidden">
        <div class="p-10 md:p-12 flex flex-col items-center gap-8 text-center">
            ...
            <div class="w-full flex flex-col sm:flex-row justify-center gap-4 border-t border-outline pt-8">
                <?php if(auth()->check()): ?>
                    <a href="<?php echo e(route('dashboard')); ?>" class="h-14 px-8 bg-primary text-white rounded-2xl font-bold text-sm hover:shadow-xl hover:shadow-primary/20 transition-all flex items-center justify-center gap-2 active:scale-[0.98]">
                        <span class="material-symbols-outlined">home</span>
                        Ir al Dashboard
                    </a>
                    <form action="<?php echo e(route('logout')); ?>" method="POST" class="w-full sm:w-auto">
                        <?php echo csrf_field(); ?>
                ...
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="h-14 px-8 bg-primary text-white rounded-2xl font-bold text-sm hover:shadow-xl hover:shadow-primary/20 transition-all flex items-center justify-center gap-2 active:scale-[0.98]">
                        <span class="material-symbols-outlined">login</span>
                        Iniciar Sesión
                    </a>
                    <a href="<?php echo e(route('register')); ?>" class="h-14 px-8 border border-outline text-secondary hover:bg-surface-variant rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2">
